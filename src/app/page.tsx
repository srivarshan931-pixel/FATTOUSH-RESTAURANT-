...
import { Clock, Compass, Award, Star, ArrowRight, CheckCircle2, Play, Pause, MapPin, Phone, Shield } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    // Only attempt video playback on desktop/tablet size (>= 768px)
    ...
    // Interaction fallback for restrictive mobile browsers / battery saver modes
    const enableVideoPlay = () => {
      attemptPlay();
      window.removeEventListener("touchstart", enableVideoPlay);
      window.removeEventListener("scroll", enableVideoPlay);
    };

    window.addEventListener("touchstart", enableVideoPlay, { passive: true });
    window.addEventListener("scroll", enableVideoPlay, { passive: true });
  ...
  useGSAP(
    () => {
      ...
      showcaseItems.forEach((item) => {
        ...
        const img = element.querySelector(".showcase-img");
        gsap.fromTo(
          img,
          { yPercent: -12 },
          {
      ...
      fadeTitles.forEach((title) => {
        const element = title as Element;
        gsap.fromTo(
          element,
          { opacity: 0, y: 50 },
          {
  ...
  const handleBookingSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!result.success) {
        setBookingError(result.error || "An unexpected error occurred. Please try again.");
        return;
      }
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden bg-forest-dark min-h-screen">
      ...
      <section ref={heroRef} id="hero" className="relative h-screen w-full flex items-center justify-between overflow-hidden px-4 sm:px-8 md:px-16 lg:px-24">
        ...
        <div className="absolute inset-0 z-0 hero-bg-parallax w-full h-[135%] -top-[20%]">
          ...
          <video
            ...
            muted
            playsInline
            className="hidden md:block w-full h-full object-cover filter brightness-[0.35] saturate-[0.7] contrast-[1.05]"
          >
            <source
          ...
          {/* Fallback Static Image (Mobile only) */}
          <img
            src="/assets/intro/hero_mobile_bg.png"
            alt="Fattoush Fine Dining"
            fetchPriority="high"
            className="block md:hidden w-full h-full object-cover filter brightness-[0.35] saturate-[0.7] contrast-[1.05]"
          />
          ...
          <div className="absolute inset-0 border-x border-white/10 mx-4 sm:mx-8 md:mx-16 lg:mx-24 pointer-events-none" />
          <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-white/5 pointer-events-none" />
        </div>

        {/* Left Side: Typography */}
        <div className="relative z-10 max-w-4xl w-full flex flex-col items-start text-left pt-16">
          <div className="overflow-hidden mb-4">
            <span className="hero-reveal block text-[10px] sm:text-xs md:text-sm font-sans tracking-[0.22em] sm:tracking-[0.35em] md:tracking-[0.45em] text-gold-warm font-black uppercase">
              RESTORING TASTE SINCE 2016
            </span>
          </div>

          <div className="overflow-hidden mb-6 w-full">
            <h1 className="hero-reveal font-display text-[1.95rem] min-[360px]:text-[2.25rem] min-[400px]:text-[2.6rem] sm:text-6xl md:text-8xl lg:text-9xl font-black tracking-wider min-[400px]:tracking-widest text-white leading-[0.9] sm:leading-[0.85] pr-6 md:pr-10 lg:pr-16">
              FATTOUSH
            </h1>
          </div>

          <div className="overflow-hidden mb-10">
            <p className="hero-reveal text-xs sm:text-sm md:text-base tracking-[0.08em] sm:tracking-[0.15em] md:tracking-[0.2em] text-gold-pale/80 max-w-lg leading-relaxed font-sans uppercase">
              Immersive Arabian grills, Malabar curries, & tandoori classics.
            </p>
          </div>

          <div className="overflow-hidden flex flex-col min-[375px]:flex-row gap-3 w-full min-[375px]:w-auto">
            <button
              onClick={() => document.getElementById("menu")?.scrollIntoView({ behavior: "smooth" })}
              className="hero-reveal w-full min-[375px]:w-auto px-6 py-3.5 sm:px-8 sm:py-4 bg-gold-warm text-forest-dark text-[10px] sm:text-xs tracking-[0.15em] sm:tracking-[0.25em] font-sans font-bold hover:bg-white hover:text-forest-dark transition-all duration-500 rounded-sm cursor-pointer shadow-lg uppercase text-center justify-center"
            >
              Discover Menu
            </button>
            <button
              onClick={() => document.getElementById("booking")?.scrollIntoView({ behavior: "smooth" })}
              className="hero-reveal w-full min-[375px]:w-auto px-6 py-3.5 sm:px-8 sm:py-4 bg-transparent border border-white/40 text-white text-[10px] sm:text-xs tracking-[0.15em] sm:tracking-[0.25em] font-sans font-bold hover:bg-white hover:text-forest-dark hover:border-white transition-all duration-500 rounded-sm cursor-pointer uppercase text-center justify-center"
            >
              Book Table
            </button>
          </div>
      ...
      <section id="booking" className="relative py-28 md:py-36 bg-forest-deep">
        <div className="max-w-3xl mx-auto px-6 relative z-10">
          ...
          {/* Reserve Table trigger button — hidden once form is open */}
          {!showBookingForm && !bookingSubmitted && (
            <div className="flex justify-center">
              <button
                onClick={() => setShowBookingForm(true)}
                className="px-10 py-4 bg-gold-warm text-forest-dark text-xs tracking-[0.25em] font-sans font-bold hover:bg-white hover:text-forest-dark transition-all duration-500 rounded-sm cursor-pointer uppercase shadow-lg"
              >
                Reserve Table
              </button>
            </div>
          ...
          {/* Booking form — shown after clicking Reserve Table */}
          {showBookingForm && !bookingSubmitted && (
            <form onSubmit={handleBookingSubmit} className="flex flex-col gap-8 bg-forest-dark/40 backdrop-blur-md p-8 md:p-12 border border-white/10 rounded-sm shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {/* "Just keep the reservation secure" table section */}
              ...
              >
                {bookingLoading ? "Sending..." : "Send Reservation"}
              </button>
            </form>
          )}
          ...
          {bookingSubmitted && (
            <div className="flex flex-col items-center gap-6 bg-forest-dark/40 backdrop-blur-md p-12 border border-white/15 rounded-sm shadow-2xl text-center">
              ...
              <button
                onClick={() => { setBookingSubmitted(false); setShowBookingForm(false); }}
